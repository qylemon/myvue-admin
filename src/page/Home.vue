  ...
  <div style="height: 100%;background: var(--backgroundStyle)">
    <div class="header-div disFlex">
      ...
      <div class="disFlex header-div-right">
        <div class="settingOption-div disFlex" @click="isUpdataTheme = true" style="margin-right: 18px;">
          <i class="el-icon-menu" style="font-size: 20px; padding: 0px 10px;"></i>
          <span>更换主题</span>
        </div>
          ...
          <div @click="showMenu(index)" class="menu-list-bgc disFlex">
            ...
            <div style="width: 20%;text-align: right;">
              <i
                ...
                v-if="menu.adminMenuList.length > 0"
              ></i>
            </div>
          </div>
          <el-collapse-transition>
            <div v-show="menu.isOpen==1">
              <a>
                <router-link
                  tag="li"
                  :to="menuSon.query == undefined? menuSon.path : menuSon.path + menuSon.query"
                  v-for="(menuSon,index1) in menu.adminMenuList"
                  :key="menuSon.id"
                >
                  <div
                    class="menuSon-list-bgc"
    ...
    this.menuList = [
      {
        ...
        isOpen: 0,
        adminMenuList: [
          {menuName: "查看文章", isOpen: 0, path: "/note"}
        ]
      },
      {
        menuName: "评论管理",
        isOpen: 0,
        adminMenuList: [
          {menuName: "查看评论", isOpen: 0, path: "/comment"}
        ]
      },
      {
        menuName: "用户管理",
        ...
        adminMenuList: [
          ...
          {menuName: "新增用户", isOpen: 0, path: "/adduser"}
        ]
      },
      {
        menuName: "分类管理",
        isOpen: 0,
        adminMenuList: [
          { menuName: "查看分类", isOpen: 0, path: "/tag" },
          {menuName: "新增分类", isOpen: 0, path: "/addtag"}
        ]
      },
      {
        menuName: "权限管理",
        isOpen: 0,
        adminMenuList: [
          { menuName: "查看权限", isOpen: 0, path: "/auth" },
          {menuName: "新增权限", isOpen: 0, path: ""} //增加二层菜单
        ]
      },
      {
        menuName: "数据监测",
        ...
        adminMenuList: [
          ...
          },
          {
            ...
            isOpen: 0,
            path: "/Iframe",
            query: "/http%3A%2F%2Felement-cn.eleme.io/#/zh-CN"
          },
          { menuName: "富文本demo", isOpen: 0, path: "/articleManage" },
          { menuName: "德鲁伊", isOpen: 0, path: "/druid" }
        ]
      }
    ];
    this.userName = "admin";
